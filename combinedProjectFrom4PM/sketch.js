...
var magnetRamp, magnetRamp2, magnetRamp3;
var magnetGround;
var blockingRect, blockingRect2, blockingRect3;
var magnetActivate = false, magnetActivate2 = false, magnetActivate3 = false;
var pushMagnetBall = false;
...
pulleyP = Bodies.rectangle(6300, 4100, 400, 20);
  ...
  magnetRamp = Bodies.rectangle(6900, 4200, 300, 10, {
    isStatic: true, 
    angle: Math.PI * 0.16,
    // angle: Math.PI * 0.06
  });

  World.add(engine.world, [magnetRamp]);

  magnetRamp2 = Bodies.rectangle(7150, 4450, 300, 10, {
    isStatic: true, 
    angle: Math.PI * 0.16,
    // angle: Math.PI * 0.06
  });

  World.add(engine.world, [magnetRamp2]);

  magnetRamp3 = Bodies.rectangle(7400, 4700, 300, 10, {
    isStatic: true, 
    angle: Math.PI * 0.16,
    // angle: Math.PI * 0.06
  });

  World.add(engine.world, [magnetRamp3]);

  magnetGround = Bodies.rectangle(8000, 5100, 900, 10, {
    isStatic: true
  });

  World.add(engine.world, [magnetGround]);

  magnet = Bodies.rectangle(8000, 4500, 700, 50, {
    isStatic: true
  });

  World.add(engine.world, [magnet]);

  magnetHandle = Bodies.rectangle(8000, 4275, 50, 400, {
    isStatic: true
  });

  World.add(engine.world, [magnetHandle]);

  blockingRect = Bodies.rectangle(7800, 4900, 30, 200, {
  });

  World.add(engine.world, [blockingRect]);

  blockingRect2 = Bodies.rectangle(8000, 4900, 30, 200, {
  });

  World.add(engine.world, [blockingRect2]);

  blockingRect3 = Bodies.rectangle(8200, 4900, 30, 200, {
  });

  World.add(engine.world, [blockingRect3]);
...
var col1, col2, col3, col4, col5, col6, col7, col8 = false
var collisionNum = 0;
function draw() {
  ...
  Events.on(engine, 'collisionStart', function(event) {
    ...
    if((Matter.SAT.collides(plinkoBall4, plinkoRamp).collided) && !col4){
      currentCamBody=plinkoBall4//switching camera focus
      col4 = true;
      World.remove(engine.world, projectile)
    }


    if(((Matter.SAT.collides(plinkoBall2, carBR3).collided) || (Matter.SAT.collides(plinkoBall2, carBR2).collided))&& !col5){
    ...
    if((Matter.SAT.collides(allObjectsArray[19], ballA).collided) && !col8){
    ...
    if((Matter.SAT.collides(ball2P, magnetRamp).collided)){
      magnetActivate = true;
      console.log("C1")
    }
    if((Matter.SAT.collides(ball2P, magnetRamp2).collided)){
      magnetActivate2 = true;
      console.log("C2")
    }
    if((Matter.SAT.collides(ball2P, magnetRamp3).collided)){
      magnetActivate3 = true;
      console.log("C3")
    }
    if((Matter.SAT.collides(ball2P, magnetGround).collided)){
      pushMagnetBall = true;
    }
...
if(pushMagnetBall){
  Body.applyForce(ball2P, {x: ball2P.position.x , y: ball2P.position.y} , { x: 0.0001, y: 0 });
...
renderVertices(magnetRamp);
renderVertices(magnetRamp2);
renderVertices(magnetRamp3);
renderVertices(magnetGround);

if(magnetActivate){

  Body.applyForce(blockingRect, {x: blockingRect.position.x , y: blockingRect.position.y} , { x: 0, y: -0.05 });
  
 

}
if(magnetActivate2){
  Body.applyForce(blockingRect2, {x: blockingRect2.position.x , y: blockingRect2.position.y} , { x: 0, y: -0.05 });
}
if(magnetActivate3){
  Body.applyForce(blockingRect3, {x: blockingRect3.position.x , y: blockingRect3.position.y} , { x: 0, y: -0.05 });
}


renderVertices(blockingRect);
renderVertices(blockingRect2);
renderVertices(blockingRect3);
