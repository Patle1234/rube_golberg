...
// progY=5230
  ...
  Matter.Events.on(engine,'afterUpdate', function() {
    if (firing && Math.abs(carSS.position.x-progX) < 20 && Math.abs(carSS.position.y-progY) < 20) {
      pastBall=carSS;
      currentCamBody=pastBall
      carSS=Bodies.circle(progX, progY,40)
        Matter.World.add(engine.world, carSS);
        elastic.bodyB = carSS;
        firing = false;
...
// buttonPresser =Bodies.rectangle(14890, 5100, 70,30,{
...
renderVertices(ballA)
  ...
  // console.log(carSS.position.y);
  
 
  if (Matter.SAT.collides(carSS, platform).collided) {
    console.log("colliding!")
    World.remove(engine.world,button)
    World.remove(engine.world,pastBall)
